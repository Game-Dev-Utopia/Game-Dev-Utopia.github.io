...
const Filter = ({ addToFilterList, setItem, removeFromFilterList, filterCount, clearFilters }) => {
  ...
  return (
    <div className='flex flex-wrap md:justify-center mb-10 bg-slate-900 ml-2'>
      {
        menu.map((val, i) => (
          <div
            key={i}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="flex  justify-center items-center text-center outline-none font-medium text-sm ">
                {val.title}
      ...
      <div className={`${select.Downloadable ?
        "ml-2 my-2 text-center h-12 px-5 rounded-full items-center inline-flex text-sm font-medium bg-grad text-white"
        :
        "ml-2 my-2 text-center h-12 px-5 rounded-full items-center inline-flex text-sm font-medium shadow-sm bg-slate-800 shadow-gray-600"}`}
        onClick={() => {
