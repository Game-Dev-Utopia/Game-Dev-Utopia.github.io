...
const TimelineCard = ({ data, scrollCurrent, index }) => {
  ...
  useEffect(() => {
    if (scrollCurrent && timelineCardRef) {
      var scrollVal =
        (scrollCurrent.scrollTop +
          scrollCurrent.clientHeight / 1.5 -
          timelineCardRef.current.offsetTop) /
        timelineCardRef.current.clientHeight;

      if (scrollVal > 0 && scrollVal < 1) scrollVal = 1;

      setTransparency(scrollVal);
    } else {
      if (index == 0) setTransparency(1);
    }
  });

  return (
    <div
      className={`Achievement transition-all duration-1000`}
    ...
    >
      ...
      />
      <p className="text-center Achievement-time text-sm md:text-lg rounded-[1rem]  border-2 px-3 bg-black">
        {data.date}
      </p>
      ...
      <div
        style={{
          boxShadow:
            transparency >= 1
              ? "0px 0px 100px 0px rgba(6, 182, 212, 0.2)"
              : "none",
      ...
      >
        <div className="Achievement-detail-container">
          <div className="Achievement-detail-inner"></div>
