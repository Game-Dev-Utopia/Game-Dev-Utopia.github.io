...
const TimelineCard = ({ data , scrollCurrent, index }) => {
    ...
    return (
        <div className={`Achievement transition-all duration-1000`} style={{opacity : transparency}} ref={timelineCardRef}>
            ...
            />
            <p className='text-center Achievement-time text-sm md:text-lg rounded-[1rem]  border-2 px-3 bg-black'>{data.date}</p>
            <div className='Achievement-tag'></div>
            <div 
                style={{ 
                    boxShadow : (transparency >= 1) ? '0px 0px 100px 0px rgba(6, 182, 212, 0.2)' : 'none',
