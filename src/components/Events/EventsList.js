...
const EventsList = ({ title, events }) => {
  const eventslist = events.map((event) => ({
    ...
    webLink: event.webUrl,
    date: event.eventDate,
  }));

  return (
    <div className="my-8">
      <h3 className="text-4xl md:text-5xl font-extrabold txt-grad ml-4 leading-loose">
        {title}
      </h3>
      {eventslist.length === 0 ? (
        <p className="text-center text:md md:text-lg ml-4 mt-2 text-yellow-600 font-semibold">
          We don&apos;t have any upcoming events for now. Keep visiting for
          further updates!
        </p>
      ) : (
          ...
          {eventslist.map((event, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black bg-opacity-75 opacity-0 group-hover:opacity-100 flex flex-col justify-end p-2 md:p-4 transition-opacity duration-300">
                ...
                <Link
                  target="_blank"
                  href={event.webLink || ` /events/${event.id}`}
                >
                  <button className="bg-black text-white w-full py-1 mt-2 rounded">
                    Visit
                  </button>
                </Link>
              </div>
